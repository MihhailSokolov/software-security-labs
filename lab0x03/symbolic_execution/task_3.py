...
def crc_update(buf, crc):
    ...
    for i in range(len(buf)):
        k = buf.get_byte(i)
        crc = (crc >> 8) ^ get_crc32_table_BVV((crc & 0xff) ^ k)
    return crc ^ 0xffffffff

def crc32(buf):
    return crc_update(buf, 0)


flag = claripy.BVS('flag', 16*8)
solver = claripy.Solver()

# solver.add('./crc32 flag' == 0xff09b794)
solver.add(crc32(flag) == 0xff09b794)
# solver.add(re.match('^[A-Z]{16}$', str(flag.get_bytes(0, 16))))
# solver.add(str(flag.get_bytes(0, 4)) == 'FLAG')

flag_value = solver.eval(flag, 1)[0]
print(flag_value)

with open('task_3.txt', 'w') as output:
    output.write(flag_value)